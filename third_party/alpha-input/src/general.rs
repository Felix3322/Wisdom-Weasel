...
use thiserror::Error;
use tracing::{info, debug, instrument, trace};
    ...
    for General<T>
{
    ...
    fn get_predict_vector(&self, input: &str) -> Result<Array2<T>, Self::Error> {
        ...
        debug!("Token count: {}. Max input length: {}.", token_count, self.max_input_length);
        if token_count > self.max_input_length {
            info!("Input too long: {} tokens (max: {}).", token_count, self.max_input_length);
            return Err(GeneralError::InputTooLong(token_count, self.max_input_length));
        }
        ...
        let last_hidden_state = outputs[0].try_extract::<f32>().map_err(GeneralError::TensorExtraction)?;
        let last_hidden_state_view = last_hidden_state.view();
        trace!("Last hidden state extracted. Shape: {:?}", last_hidden_state_view.shape());
        ...
        debug!("Extracted last token vector. Shape: {:?}", last_token_vec.shape());
...
impl<T: TypeToTensorElementDataType + Clone + TensorDataToType + FromPrimitive> General<T> {
    ...
    pub fn new(model_path: &str, tokenizer_path: &str, optimization_level: i32, max_input_length: usize, inference_hardware: &str) -> Result<Self, GeneralError> {
        info!("Initializing General model with optimization_level: {}, max_input_length: {}, inference_hardware: {}", optimization_level, max_input_length, inference_hardware);
    ...
    fn load_session(model_path: &str, optimization_level: i32, _inference_hardware: &str) -> Result<Session, GeneralError> {
        ...
        let environment = Arc::new(Environment::builder().with_name("test").build().map_err(GeneralError::EnvironmentCreation)?);
        debug!("Environment created. Building session...");
        let mut builder = SessionBuilder::new(&environment).map_err(GeneralError::SessionBuilder)?;
        ...
        builder = builder.with_optimization_level(opt_level).map_err(GeneralError::SessionBuilder)?;
    
        #[cfg(feature = "cuda")]
        if inference_hardware == "cuda" {
            builder = builder.with_execution_providers(vec![ort::execution_providers::CUDAExecutionProvider::default()])
                .map_err(GeneralError::SessionBuilder)?;
        }

        let session = builder.with_model_from_file(model_path).map_err(GeneralError::ModelLoading)?;
    ...
    fn load_tokenizer(tokenizer_path: &str) -> Result<Tokenizer, GeneralError> {
        ...
        if !std::path::Path::new(tokenizer_path).exists() {
            ...
            return Err(GeneralError::TokenizerFileNotFound(tokenizer_path.to_string()));
        }
