...
use lmdb_zero::{Environment as LmdbEnv, Database};
use ndarray::Array1;
use thiserror::Error;
...
pub enum DatabaseError {
    ...
    TokenNotFound(u32),
    #[error("Invalid embedding size for token {token_id}: expected {expected} bytes, found {found}")]
    InvalidEmbeddingSize { token_id: u32, expected: usize, found: usize },
...
impl LmdbManager {
    ...
    pub fn open(db_path: &str, map_size_mb: usize, read_only: bool) -> Result<Self, DatabaseError> {
        ...
        let flags = if read_only { lmdb::open::RDONLY } else { lmdb::open::Flags::empty() };
        let map_size = map_size_mb * 1024 * 1024;
        
        debug!("LMDB environment flags: {:?}, map_size: {} bytes", flags, map_size);
        let env: Arc<LmdbEnv> = Arc::new(unsafe {
            ...
            builder.set_mapsize(map_size)
                .map_err(|e| DatabaseError::EnvironmentError(e.to_string()))?;
            builder.open(db_path, flags, 0o600)
                .map_err(|e| DatabaseError::EnvironmentError(e.to_string()))?
        });
        ...
        info!("Database loaded - vocab_size: {}, embedding_dim: {}, quantize: {:?}", 
                info.vocab_size, info.embedding_dim, quantize_type);
        
        Ok(Self { env, db, info, quantize_type })
    }
    
    fn read_metadata(env: &Arc<LmdbEnv>, db: &Database<'static>) -> Result<EmbeddingInfo, DatabaseError> {
        debug!("Reading metadata from LMDB.");
        let txn = lmdb::ReadTransaction::new(env.clone())
            .map_err(|e| DatabaseError::TransactionError(e.to_string()))?;
        let access = txn.access();
        let meta_bytes: &[u8] = access.get(db, b"__meta__")
    ...
    fn read_quantize_type(env: &Arc<LmdbEnv>, db: &Database<'static>) -> Result<Option<String>, DatabaseError> {
        ...
        match access.get::<_, [u8]>(db, b"__quantize__") {
            Ok(bytes) => {
                let q_type = String::from_utf8(bytes.to_vec()).map_err(|_| DatabaseError::MetadataParseError("Invalid quantize type".to_string()))?;
    ...
    pub fn get_token_embedding(&self, token_id: u32) -> Result<Array1<f32>, DatabaseError> {
        ...
        let value_bytes: &[u8] = access.get(&self.db, &key_bytes)
            .map_err(|e| {
        ...
        if let Some(qtype) = &self.quantize_type {
            match qtype.as_str() {
                "int8" => {
                    ...
                    let dequantized_vec: Vec<f32> = quantized_slice.iter()
                ...
                "int4" => {
                    ...
                    let expected_len = packed_len + 4; // 4 bytes for f32 scale
    ...
    #[instrument(skip(self, tokenizer), fields(word = word))]
    pub fn get_word_embedding(&self, word: &str, tokenizer: &tokenizers::Tokenizer) -> Result<Array1<f32>, DatabaseError> {
        ...
        for &token_id in &token_ids {
            let emb = self.get_token_embedding(token_id)?;
            embeddings.push(emb);
        }
        debug!("Retrieved {} embeddings for word '{}'.", embeddings.len(), word);
        ...
        if embeddings.len() == 1 {
            ...
            for emb in &embeddings {
                sum_embedding = sum_embedding + emb;
            }
